    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <BaseCard v-for="level in levels" :key="level.id" class="p-4">
        ...
        <BaseButton 
          ...
          :class="{ 'opacity-50 cursor-not-allowed': level.name !== 'Beginner' && !userCanAccessLevel(level) }"
...
onMounted(async () => {
  try {
    const response = await store.dispatch("levels/fetchLevels");
...
const userCanAccessLevel = (level: any) => {
